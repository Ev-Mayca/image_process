...
extern GPS_INFO gps;
extern GPS_GSV_INFO gps_gsv;
extern GPS_GSV_INFO gps_gsv1;
extern GPS_GSV_INFO gps_gsv_total;
//extern GPS_GSA_INFO gps_gngsa;
extern GPS_GSA_INFO gps_gsa[2];
extern GPS_GSA_INFO gps_gsa_total;
...
Draw draw;
Mat sat_image(w, w, CV_8UC3);
Mat ned_pic(w, w, CV_8UC3);
Mat parameter_image(MAX_WIDTH + MAX_EDGE_X, MAX_HEIGHT + MAX_EDGE_Y, CV_8UC3);
...
int main(int argc, char **argv)
{
    ...
    char rcv_buf[512];
    double max_data = 0;
    double data_buf_p[MAX_WIDTH];
    double data_buf_v[MAX_WIDTH];
    double data_buf_h[MAX_WIDTH];
    ...
    while (1) //循环读取数据
    {
        ...
        } while (gpss.process_gps(rcv_buf, &gps_temp) != 6);
